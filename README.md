...
| :-: | :-: | :-: | :-: |:-: |:-: |
| 缺页数 | 11 | 17 | 30 | 14 | 14 |
| 缺页率 | 0.343750 | 0.531250 | 0.937500 | 0.437500 | 0.437500 |

| 算法名称 | 最佳置换算法 | 先进先出算法 | 最近最久未使用算法 | 改进型Clock置换算法 | 页面缓冲置换算法 |
| :-: | :-: | :-: | :-: |:-: |:-: |
| 缺页数 | 14 | 22 | 38 | 16 | 19 |
| 缺页率 | 0.437500 | 0.687500 | 1.187500 | 0.500000 | 0.593750 |
>>我们可以看出，物理内存越大，缺页率越小；内存尺寸越大，缺页率就越小<br>
### 2.算法比较<br>
* 从缺页率可以看出，最佳置换算法效果最佳，最近最久未使用算法最差，最佳置换算法会比其他算法的性能好很多，但由于这个算法是一种理想的算法，实际上难于实现，主要用于参考。<br>
* 性能方面，先进先出和改进后的clock算法性能基本相同，两种 clock 算法遍历链表采用的就是 FIFO 的方法，而改进的 clock 算法相比于简单 clock 算法的优势主要体现在会根据是否被修改进行选择，以减少写回所花费的时间。<br>
## 五、实验心得<br>
通过本次实验，然后给我对置换算法有了更深刻的认识，通过实验，每个算法各自的优缺点更加的直观，另一个收获是了解了一些生成数据的方法，总体来说，本次实验题目虽然较多，但是思路基本相同，实现起来并不困难。
